...
mask_all_channel_values_30to70 = np.asarray(apply_masks_and_average(hys_img,mask_30_to_70))
mask_all_channel_values_85 = np.asarray(apply_masks_and_average(hys_img,mask_above_85))

#%% Perform PCA to see if the grups differ from one another
import PCA_AnalysisModule
road_data = hys_img[segment_mask > 1]
PCI_data = segment_mask[segment_mask > 1]
fitted_data_idx, data_PCA2, Kmeans_labels = PCA_AnalysisModule.pca_kmeans_clustering(road_data, n_clusters=6)
fitted_hysData = road_data[fitted_data_idx, :]
fitted_PCIData = PCI_data[fitted_data_idx]

## Train SVM
PCI_labels = np.zeros(np.shape(fitted_PCIData))
PCI_labels[fitted_PCIData > 70] = 1
# PCI_labels[fitted_PCIData > 80] = 2 # Need more data/more PCA.. Needs Fixing 
svm_classifier, metrics = PCA_AnalysisModule.train_svm_classifier(data_PCA2, PCI_labels, use_grid_search=True)
# PCA_AnalysisModule.plot_svm_decision_boundaries_3d(svm_classifier, data_PCA2, PCI_labels)

PCI_HistFig = plt.figure()
PCA_scatterFig = plt.figure()
cmap = get_lighttraffic_colormap()
# Plot PCA with PCI colors

# Create 3D scatter plot
fig = plt.figure(figsize=(8, 6))
ax = fig.add_subplot(111, projection='3d')
sc = ax.scatter(data_PCA2[:, 0], data_PCA2[:, 1], data_PCA2[:, 2], c=fitted_PCIData, cmap=cmap)

# Add color bar
cbar = plt.colorbar(sc, ax=ax, pad=0.1)
cbar.set_label('PCI (0 to 100)')
plt.xlabel('PC 1')
plt.ylabel('PC 2')
ax.set_zlabel('PC 3')
plt.title('PCA of Road Pixel Data')
