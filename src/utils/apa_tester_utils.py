...
class PavementDataProcessor:
    def __init__(self, config):
        self.config = config
        self.X_cropped=None
        self.Y_cropped = None
        self.ROI_coords = None
        self.cropped_rect = None
        self.RGB = None
        self.HSI = None
        self.segID_mask = None
        self.all_roads_mask = None

    @staticmethod
    def image_data_loader(config, data_dirname, data_filename, metadata_filename):
        lon_mat, lat_mat, VenusImage = ImageHandler.get_hypter_spectral_imaginery(data_filename, data_dirname, config["data"])
    ...
    def import_data(self, lon_mat, lat_mat, VenusImage, excel_path, roi):
        if self.config["data"]["zone"] == "Israel":
            GT_xy_PCI = PavementDataProcessor.get_GT_xy_PCI(excel_path, isLatLon=False)  # convert to Israel Coord to LatLon
            points_PCI, ROI_point_idx = PavementDataProcessor.get_PCI_ROI(roi, GT_xy_PCI[:3])
            ROI_seg = []
        else:
            GT_xy_PCI = PavementDataProcessor.get_GT_xy_PCI(excel_path, isLatLon=True)
            points_PCI, ROI_point_idx = PavementDataProcessor.get_PCI_ROI(roi, GT_xy_PCI[:3])
            seg_id = GT_xy_PCI[-1]
            ROI_seg = seg_id[ROI_point_idx]

        # USE only relevant ROI
        X_cropped, Y_cropped, hys_img, RGB_enchanced, cropped_rect = ImageHandler.cropROI_Venus_image(roi, lon_mat, lat_mat,

                                                                                         VenusImage)
        self.ROI_coords=roi
        self.X_cropped = X_cropped
        self.Y_cropped=Y_cropped
        self.HSI=hys_img
        self.RGB=RGB_enchanced
        self.cropped_rect=cropped_rect
        self.GT_xy_PCI=GT_xy_PCI
        self.points_PCI=points_PCI
        self.ROI_point_idx = ROI_point_idx
        self.seg_id = seg_id
        self.ROI_seg = ROI_seg
    ...
    def get_GT_xy_PCI(xls_path, isLatLon=False):
        ...
        if xls_suffix == '.csv':
            ...
            raise ValueError(
                f"Unsupported file extension: '{xls_suffix}'. Supported extensions are: {['.csv', '.xls']}")
    ...
    def create_roads_segID_mask(self):
        OC_xy = self.cropped_rect[:2]
        ...
        points_merge_PCI = pc_utils.merge_close_points(self.points_PCI[:, :2], self.points_PCI[:, 2], 50e-5)  # TODO:
        xy_points_merge = points_merge_PCI[:, :2]

        # scatter_plot_with_annotations(points_PCI,ax_roi)
        binary_mask = np.zeros(self.HSI.shape[:-1])

        roads_mask=MaskGenerator(self.config,self.X_cropped, self.Y_cropped, self.ROI_coords, self.cropped_rect)

        # USE open street maps for creating map of all
        if 'osx_map_mask_path' in self.config["preprocessing"]["georeferencing"]:
            npz_filename = self.config["preprocessing"]["georeferencing"]["osx_map_mask_path"]
        else:
        ...
        coinciding_mask = roads_mask.get_mask_from_roads_gdf(npz_filename)

        lut = None
        if len(self.ROI_seg) == 0:  # if there is not segemts ID with the PCI data, use the old method for building mask for roads
            ...
            extended_mask = roads_mask.create_proximity_mask(xy_points_merge)
            ...
            grid_value = griddata(self.points_PCI[:, :2], self.points_PCI[:, 2], (self.X_cropped, self.Y_cropped), method='nearest')
            classified_roads_mask = grid_value * combine_mask_roads
        else:
            # Dijkstra merge point
            if 'dijkstra_map_mask_path' in self.config["preprocessing"]["georeferencing"]:
                npz_filename = self.config["preprocessing"]["georeferencing"]["dijkstra_map_mask_path"]
            else:
                npz_filename = 'data/Detroit/masks_OpenStreetMap/Detroit_dijkstra_roads_mask.npz'
            npz_filename = os.path.join(REPO_ROOT, npz_filename)
            merge_points_dijkstra, lut = pc_utils.merge_points_dijkstra(npz_filename, self.X_cropped, self.Y_cropped,
                                                                        coinciding_mask,
                                                                        self.points_PCI, self.ROI_seg)

            x_ind_min, y_ind_min, x_ind_max, y_ind_max = self.cropped_rect
...
class ImageHandler:
    ...
    def get_hypter_spectral_imaginery(data_filename, data_dirname, config_data):
        if config_data['zone'] == "Detroit" and config_data['big_tiff'] == False:
            ...
            x = getLatLon_fromTiff.convert_raster_to_geocoords(os.path.join(data_dirname, data_filename),
                                                               zone_number=17, zone_letter='T')
            # unpack lat/lon
            lon_mat = x[:, :, 0]
            lat_mat = x[:, :, 1]
    ...
    def vis_hys_image(hys_img, channels='rgb'):
        ...
        if isinstance(channels, str) and channels == 'rgb':
            ...
            return np.stack((hys_img[:, :, channels[0]], hys_img[:, :, channels[1]], hys_img[:, :, channels[2]]),
                            axis=-1)
        elif len(channels) == 1:
    ...
    def cropROI_Venus_image(roi, lon_mat, lat_mat, VenusImage):
        ...
        for normBandIdx in range(len(norm_vec)):
            ...
            RGB_Img[:, :, normBandIdx] = ImageHandler.equalize_image(img, 1)
...
class MaskGenerator:
    def __init__(self, config,X_Grid,Y_Grid,crop_coords,crop_rect):
        self.config = config
        self.X_Grid = X_Grid
        self.Y_Grid = Y_Grid
    ...
    def create_proximity_mask(self,xy_points_merge, threshold=10e-5, *arg):
        # Build a KD-tree for the features
        tree = cKDTree(xy_points_merge)
        # Flatten grid points for querying
        grid_points = np.vstack((self.X_Grid.ravel(), self.Y_Grid.ravel())).T
        ...
        KDTree_mask = (distances <= threshold).astype(int).reshape(self.X_Grid.shape)

        return KDTree_mask


    @pc_utils.log_execution_time
    def get_mask_from_roads_gdf(self,npz_filename):
        ...
        if not (os.path.exists(npz_filename) and os.path.exists(metda_filename)):
            if self.Y_Grid is None:
                ...
                self.create_mask_from_roads_gdf(npz_filename)
        else:
            with open(metda_filename, 'r') as f:
                metadata_dict = json.load(f)
            is_roi_within_bounds = pc_utils.is_roi_within_bounds(self.crop_coords, metadata_dict["roi"])
            if not is_roi_within_bounds:
                raise ValueError(
                    f"Exisiting ROI:{metadata_dict["roi"]}  of GDF roads is not bounded by the requested: {self.crop_coords}")
            ...
            x_ind_min, y_ind_min, x_ind_max, y_ind_max = self.crop_rect
            coinciding_mask=coinciding_mask[y_ind_min: y_ind_max, x_ind_min: x_ind_max]
            self.all_roads_mask=coinciding_mask
            return coinciding_mask[y_ind_min: y_ind_max, x_ind_min: x_ind_max]

    @pc_utils.log_execution_time
    def create_mask_from_roads_gdf(self,npz_filename):
        roi = self.crop_coords
        lon_mat = self.Y_Grid
        lat_mat = self.X_Grid
        xmin_cut, xmax_cut, ymin_cut, ymax_cut = roi
    ...
    def fill_segement_pixels_to_curves(self,GT_xy_PCI):
        ...
        inside_mask = inside.reshape(self.X_Grid.shape)
        fill_mask = np.zeros_like(self.X_Grid, dtype=np.uint8)
        ...
        for x, y in polygon_coords:
            ...
            pixel_indices.append(np.unravel_index(idx, self.X_Grid.shape))

        # Create a blank mask and draw the line
        height, width = self.X_Grid.shape
        ...
        plt.pcolormesh(self.X_Grid, self.Y_Grid, pc_utils.morphological_operator(mask, 'dilation',
                                                                             'square',
                                                                             20))
        ###
    ...
    def enhance_mask_grad(self,grad_threshold, classified_roads_mask, RGB_enchanced, segment_mask_nan):
        ...
        _, _, mag = pc_utils.sobel_gradient(
            pc_utils.mean_filter(Y, 3))  # do sobel over mean filter luminace visible image
            ...
            if enhance_morph_operator_type is not None and enhance_morph_operator_size > 0:
                segment_mask_nan = pc_utils.nan_arr(
                    pc_utils.morphological_operator_multiclass_mask(classified_roads_mask, enhance_morph_operator_type,
                                                                    'square', enhance_morph_operator_size))
            else:
