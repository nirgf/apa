...
def processImageFile(org_image: np.ndarray,
    ...
    print("Processing image from array input...")
...
if __name__ == "__main__":  
    def AliModel(image, B, C):
        B = 70    
        C = 2     
        adjusted_image = cv.convertScaleAbs(image, alpha=C, beta=B)
        return adjusted_image
    
    data = np.load("/Users/ben/Desktop/Ben/apa/data/Detroit/masks_OpenStreetMap/cropped_msp_img.npz")
    cropped_msp_img = data["cropped"]

    x1, x2 = 2000, 2824
    y1, y2 = 3000, 3580
    cropped_img = cropped_msp_img[:, :, :3]
    image = cropped_img[y1:y2, x1:x2]

    image_uint8 = (image * 255).astype(np.uint8)
    org_image = cv.cvtColor(image_uint8, cv.COLOR_RGB2BGR)
    
    _, _, image_clear = processImageFile(image=org_image, ab_threshold = 0, save = True, verbose = False)
    brg_image = AliModel(image_clear, 70, 2)
    rgb_image = cv.cvtColor(brg_image, cv.COLOR_BGR2RGB)    
    _, _, image_clear = processImageFile(image=rgb_image, ab_threshold = 0, save = True, verbose = False)
# %%
